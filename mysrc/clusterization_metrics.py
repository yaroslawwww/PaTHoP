...
if __name__ == "__main__":
    ...
    results = compute_metrics_parallel(X, labels, n_cores=12)

    # Определим направления для метрик
    optimization_direction = {
        'rmsstd': "↓ (minimize: smaller values = more compact clusters)",
        'r2': "↑ (maximize: closer to 1 = better variance explanation)",
        'modified_hubert': "↑ (maximize: higher correlation = better clustering)",
        'calinski_harabasz': "↑ (maximize: higher ratio = better separation/compactness)",
        'i_index': "↑ (maximize: higher values = better clustering)",
        'dunn_index': "↑ (maximize: higher ratio = better separation)",
        'silhouette': "↑ (maximize: closer to 1 = better defined clusters)",
        'davies_bouldin': "↓ (minimize: smaller values = better cluster separation)",
        'xie_beni': "↓ (minimize: smaller values = better fuzzy clustering)",
        'sd_index': "↓ (minimize: smaller values = better cluster validity)",
        'sdbw_index': "↓ (minimize: smaller values = better density-based clustering)",
        'cvnn2_index': "↓ (minimize: smaller values = better neighborhood consistency)"
    }

    # Группируем метрики по типу оптимизации
    minimize_metrics = []
    maximize_metrics = []

    for name, value in results.items():
        if '↓' in optimization_direction[name]:
            minimize_metrics.append((name, value, optimization_direction[name]))
        else:
            maximize_metrics.append((name, value, optimization_direction[name]))

    # Выводим результаты с пояснениями
    print("=" * 80)
    print("METRICS TO MAXIMIZE (higher values = better clustering)")
    print("=" * 80)
    for name, value, direction in sorted(maximize_metrics):
        print(f"{name:>22}: {value:10.4f} | {direction}")

    print("\n" + "=" * 80)
    print("METRICS TO MINIMIZE (lower values = better clustering)")
    print("=" * 80)
    for name, value, direction in sorted(minimize_metrics):
        print(f"{name:>22}: {value:10.4f} | {direction}")
    ...
    print("=" * 80)
    print("For spherical clusters:")
    print("- Calinski-Harabasz and Davies-Bouldin are most reliable")
    print("- Look for high values in R-squared and Silhouette")
    print("\nFor complex-shaped clusters:")
    print("- S_Dbw, Dunn's index and CVNN2 are more appropriate")
    print("- Lower values in S_Dbw and Xie-Beni indicate better clustering")
    print("\nGeneral recommendations:")
    print("- Combine multiple metrics for robust validation")
    print("- Consider cluster shape when selecting evaluation metrics")
    print("- Metrics marked with ↓ should be minimized, ↑ maximized")